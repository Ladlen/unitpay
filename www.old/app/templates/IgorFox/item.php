	...
	<div class="popup" style="height: 400px;">
		...
		<div aria-hidden="false" class="modal fade in" id="paymodal" style="display: none; margin: 0px auto; overflow: hidden; position: absolute; width: 570px; top: 60px;">
			...
			<center>
				...
				<div class="payfoot modal-footer" style="position: relative;top: 6px;">
			...
			<div style="color:#000000;padding: 10px 35px 10px 15px;margin-top: 52px;background-color: #E4D7D7;border: 1px solid #fbeed5;border-radius: 4px;margin-left: 30px;" class="alert alert-danger">
		...
		<div id="selectPay" style="display:block;">
			...
			<div style="margin-left: 56px;" id="paym" class="tabs"> 
				...
				<div id="<?=$name;?>" style="display: none;"> 
					<br />
					<center>
						<input type="submit" class="buy buy_game btn btn-default pull-left" onclick="setWayForMoney('<?=$name;?>'); setEmail();" id="setEmailButton" data-dismiss="modal" aria-hidden="true" data-toggle="modal" value="перейти к оплате" style="width: 435px;position: relative;right: -21px;top: -10px;height: 61px;background: #008fe9;    color: #ada;" />	
					</center>
				</div>
				<? } ?>
				<? } ?>
	...
	<div class="store-games" style="width: 1000px; margin: 0 auto;">
        <div class="store-one">
			<h1><?=$ITEM['item'];?></h1>
			<br />
        ...
        <div class="store-two">
            <div class="store-cover" style="background-image: url('<?=$ITEM['image'];?>')">Купить <?=$ITEM['item'];?></div>
            <div class="help toolbar"></div>
            <a id="oplata" href="#pay">
				<button type="submit" class="buy">Купить за <?=$price['WMR'];?> RUB.</button>
			</a>
		</div>
	</div>
	<? } else { ?>
