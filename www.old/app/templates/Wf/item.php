	...
	<div class="popup" style="height: 350px;">
		...
		<div aria-hidden="false" class="modal fade in" id="paymodal" style="display: none; margin: 0px auto; overflow: hidden; position: absolute; width: 570px; top: 60px;">
			...
			<center>
				<table class="paytable" style="font-size: 16px; position: relative; top: -25px; width: 60%; font-size: 9pt;">
					<tbody>
						<tr>
			...
			<div style="color:#000000;padding: 10px 35px 10px 15px;margin-top: -8px;background-color: #E4D7D7;border: 1px solid #fbeed5;font-size: 9pt;margin-left: 25px;border-radius: 4px;" class="alert alert-danger">
		...
		<div id="selectPay" style="display:block; color:black;">
			...
			<div style="margin-left: 56px;" id="paym" class="tabs"> 
				...
				<div id="<?=$name;?>" style="display: none;"> 
					<br />
					<center>
						<input type="submit" class="buy_game btn btn-default pull-left" onclick="setWayForMoney('<?=$name;?>'); setEmail();" id="setEmailButton" data-dismiss="modal" aria-hidden="true" data-toggle="modal" value="перейти к оплате" style="width: 437px;position: relative;right: 32px;top: -5px;" />	
					</center>
				</div>
				<? } ?>
				<? } ?>
	...
	<div class="sider_center">
		<article class="itemf">
			...
			<div class="itemf_info">
				<div class="heads">
					<div class="itemf_title">
						<h1><?=$ITEM['item'];?></h1>
					</div>
				</div>
				<p class="itemf_price" style="position: relative; bottom: 15px;">
					<span>ЦЕНА: <?=$price['WMR'];?> РУБ.</span>
				</p>
				<div class="itemf_buy">
					<a class="btn fbox" href="#pay">
						Купить товар
					</a>
				</div>
			</div>
			<div class="itemf_desc idesc"> 
				<p class="titles">ОПИСАНИЕ:</p>
				<center>
					<?=$ITEM['body'];?>
				</center>
			</div>
