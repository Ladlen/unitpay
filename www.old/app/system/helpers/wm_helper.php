...
function check_payment($wmid,$wm_pass,$fund,$wm_path,$desc,$amount)
{
	if(!empty($wmid) && !empty($wm_pass) && !empty($fund) && !empty($wm_path) && !empty($desc) && !empty($amount))
	{
		...
		$res = $wmxi->X3(
			...
			0,              # номер счета (в системе WebMoney) по которому выполнялась операция
			0,              # номер счета
			date('Ymd', strtotime('-3 days')),       # минимальное время и дата выполнения операции
		...
		if($resp['operations']['@attributes']['cnt'] == 1) {
			if($resp['operations']['operation']['desc'] == $desc && number_format($oper['amount'], 2, ".", "") == $amount && $resp['operations']['operation']['opertype'] == 0)
			{
				return TRUE;
			}
		}
		elseif (count($resp['operations']['operation']) > 1)
		{
			...
			{
				if($oper['desc'] == $desc && number_format($oper['amount'], 2, ".", "") == $amount && $oper['opertype'] == 0)
				{
					return TRUE;
				}
			}
