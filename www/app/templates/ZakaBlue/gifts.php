		...
		<?
			...
			if($GIFT['winner'] == "" && $GIFT['time'] < time()){
				...
				# Победитель
				mysqli_query($this->connectMainBD, "UPDATE `gifts` SET `winner` = '".mysqli_real_escape_string($this->connectMainBD, $this->connectMainBD, $winner)."' WHERE `sid` = '".intval(SID)."' AND `gid` = '".intval($GIFT['gid'])."'");
			}
			# Пришли с ulogin
		...
		<div class="big-info" style="width: 771px; margin: 0px auto; position: relative; top: 0px; left: 29px;">
			<div class="inf-left">
				<div class="gallery-block-one">
					<div class="gallery" style="overflow: hidden; width: 475px; height: 267px;">
						<img src="<?=$GIFT['image'];?>" style="display:inline;height:267px;width:475px;">
					</div>
					<div class="buy-block" style="padding: 20px; background: rgb(51, 51, 51) none repeat scroll 0% 0%; height: 227px; width: 256px; margin-top: -267px; margin-left: 475px;">
						<? if($GIFT['winner'] != ""){ ?>
						<div style="padding: 10px 20px;">
							...
							<a style="text-decoration: none;" href="<?=$GIFT['winner'];?>" target="_blank">
								...
								<div style="border-radius: 3px; padding: 5px; background: rgba(255, 255, 255, 0.09) none repeat scroll 0% 0%; height: 50px;">
									<img style="border-radius: 3px;" class="avavk" src="<?=$obj['response'][0]['photo'];?>" />
									<div style="font-size: 10pt; margin-left: 65px; margin-top: -42px; color: #fff; font-weight: bold; text-decoration: none; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;"><?=$obj['response'][0]['first_name'].' '.$obj['response'][0]['last_name'];?></div>
								</div>
							</a>
						</div>
						<? } else { ?>
						...
						<div style="margin-top: 15px;">  
							<button class="clicked" id="sharebutton" style="height: 30px; margin-left:10px;">Поделиться</button>
							<button style="display: none; height: 30px;  margin-left:10px;" id="confirmbutton" onclick="confirmgift();">Участвовать</button>
						</div>
						...
						<div data-ulogin-inited="1452475801162" style="" id="uLogin" data-ulogin="display=buttons;fields=first_name,last_name,photo;providers=vkontakte;hidden=;redirect_uri=http://<?=$_SERVER['HTTP_HOST'];?><?=$_SERVER['REQUEST_URI'];?>">
							<button class="gift_enter" id="confirmbuttongifts" style="font: bold 15px/14px arial; height: 30px; margin-left:10px;" data-uloginbutton="vkontakte">Участвовать</button>
						</div>
						<? } ?>
					</div>
				</div>
		...
		<div class="row">
			...
			<div class="row-info">
				...
				<div class="title">
					<a href="<?=$row['image'];?>"></a>
					<a href="/gifts/<?=$row['gid'];?>" style="position: relative; bottom: 4px;"><?=$row['gift'];?></a>
				</div>
				<div class="category">
					<img class="avavk" src="<?=$obj['response'][0]['photo'];?>" style="width:15px; height:15px;">
