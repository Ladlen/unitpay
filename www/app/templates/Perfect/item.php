	...
	<aside id="side-center">
		...
		<div class="popup" style="height: 370px;">
			<div class="title_modal">Покупка <?=$ITEM['item'];?></div>
			<div aria-hidden="false" class="modal fade in" id="paymodal" style="display: none; width: 560px; margin: 0px auto; overflow: hidden; position: absolute; top: 60px;">
				<div class="select_pay">Проверка оплаты:</div>
				<center>
					<table class="paytable" style="top: -25px; position: relative; font-size: 16px; height: auto; width: 80%;">
						<tbody>
							<tr>
							...
							<tr>
								<td style="color:#000000; width: 250px;">Кол-во:</td>
					...
					<div class="payfoot modal-footer" style="position: relative; top: -20px;">
						<input class="checkpaybtn" value="Проверить" data-loading-text="Проверяем..." class="checkpaybtn btn btn-primary" style="width: 55%; position: relative; left: 115px;" type="button" />
					</div>
				</center>
				<div style="color:#000000;padding: 10px 35px 10px 15px; margin-top:-45px;   background-color: #E4D7D7;  border: 1px solid #fbeed5;  border-radius: 4px;" class="alert alert-danger">
			...
			<div id="selectPay" style="display:block;">
				...
				<div style="margin-left: 56px; margin-top: -18px;" id="paym" class="tabs"> 
					...
					<div id="<?=$name;?>" style="display: none; margin-bottom:100px;"> 
						<br />
						<center>
							<input type="submit" class="buy_game" onclick="setWayForMoney('<?=$name;?>'); setEmail();" id="setEmailButton" data-dismiss="modal" aria-hidden="true" data-toggle="modal" value="перейти к оплате" style="width: 427px; right: 47px; position: relative; top: -10px; left: -37px;" />
						</center>
					</div>
					<? } ?>
					<? } ?>
				...
				<div class="extra-wrap">
					<div>
						...
						<ul style="margin-top: 15px;" class="block-title1">
							<center>
								<b><a href="#pay" style="cursor: pointer; font-size: 20px; color:white;">Купить</a></b>
							</center>
						</ul>
					</div>
				</div>
