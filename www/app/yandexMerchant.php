<?
	...
	*/
	if(isset($_POST['order'])){
		...
		*/
		$SQL = mysqli_query($this->connectMainBD, "SELECT * FROM `orders` WHERE `oid` = '".intval($_POST['order'])."' AND `sid` = '".intval(SID)."' AND `wallet` = 'YAD' AND `status` = '0'");
		/*
			Заказ найден
		*/
...
<html>
	<head>
		...
		<script type="text/javascript" src="http://<?=$_SERVER['HTTP_HOST'];?>/assets/yandex/js/main.js"></script>
		<link href="http://<?=$_SERVER['HTTP_HOST'];?>/assets/yandex/css/main.css" rel="stylesheet" media="all" />
	</head>
	<body>
		<div id="dev-app">
			...
			<div class="loader" data-reactid=".0.1:0:0" style="width: 46px; height: 46px; margin: 0 auto;">
				<div class="spinner" role="progressbar" style="position: absolute; width: 0px; top: 40%;">
					<img src="http://<?=$_SERVER['HTTP_HOST'];?>/assets/yandex/img/loading.gif" />
				</div>
			</div>
		</div>
		<script>
			YANDEX['name'] = 'Merchant';
			YANDEX['wallet'] = '<?=$WALLET['YAD'];?>';
			YANDEX['link'] = <?=(isset($ORDER['bill']) ? '"'.str_replace(Array(PREFIX, "[", "]"), Array("", "", ""), $ORDER['bill']).'"' : 'false');?>;
			YANDEX['order'] = <?=(isset($ORDER['bill']) ? '"'.$ORDER['bill'].'"' : 'false');?>;
			YANDEX['amount'] = <?=(isset($ORDER['price']) ? '"'.number_format($ORDER['price'] + ($ORDER['price'] * 0.005), 2, '.', '').'"' : 'false');?>;
		</script>
	</body>
</html>